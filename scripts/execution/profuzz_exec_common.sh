...
FUZZER=$2     #fuzzer name (e.g., aflnet) -- this name must match the name of the fuzzer folder inside the Docker container
OUTDIR=$3     #name of the output folder created inside the docker container
OPTIONS=$4    #all configured options for fuzzing
...
docker run --cpus=1 -d -it $DOCIMAGE /bin/bash -c "cd ${WORKDIR} && run ${FUZZER} ${OUTDIR} '${OPTIONS}'"
